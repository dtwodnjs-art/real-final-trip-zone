...
마이페이지는 별도 신규 도메인을 만드는 방식이 아니라 기존 회원, 예약, 결제, 쿠폰, 마일리지, 찜, 문의 도메인을 사용자 기준으로 묶어 보여주는 집계 화면으로 본다.

- 내 정보 관리: 회원 상세 정보, 회원 등급, 탈퇴 처리
- 쿠폰 리스트: `USER_COUPONS` 기준 최신순 조회, 상태 확인
- 예약내역 리스트: `BOOKINGS` 기준 최신순 조회, 상태 확인
- 마일리지 내역: `USERS.MILEAGE` 집계값과 `MILEAGE_HISTORY` 원장 조회
- 위시리스트: `WISHLISTS` 기준 숙소 목록 조회, 상세 이동
- 결제 내역: `PAYMENTS` 기준 결제/환불 내역 조회
- 문의센터: `INQUIRY_ROOMS`, `INQUIRY_MESSAGES` 기준 리스트, 등록, 상세, 수정, 삭제 흐름 제공
