...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    ...
    async function loadRows() {
      try {
        ...
        if (cancelled) return;
        setRows(nextRows);
        setSelectedEmail(nextRows[0]?.email ?? null);
      } catch (error) {
        if (cancelled) return;
  ...
  const updateStatus = async (nextStatus) => {
    ...
    try {
      await updateAdminUserStatus(selectedUser.email, nextStatus);
    } catch (error) {
      ...
      <div className="dash-table-split">
        ...
        <div className="dash-action-sheet">
          ...
          <div className="dash-action-grid">
            ...
            <button type="button" className="dash-action-btn" onClick={() => updateStatus("DORMANT")} disabled={!selectedUser}>휴면</button>
